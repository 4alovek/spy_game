...
- **`adapters/web/`** — FastAPI adapter. `app.py` exposes REST (`POST /api/lobby`, `GET /api/lobby/{id}`) and a `/ws/{lobby_id}` WebSocket; `ws_manager.py` is the web analog of `broadcast_to_lobby`; `protocol.py` builds **per-user** state snapshots (roles are hidden — `build_state` is called once per socket). After every action the whole lobby state is rebroadcast and the `frontend/` (vanilla JS) re-renders. `_resolve_round` in `app.py` is the single place a round ends (the planned hook for DB stats). Identity is a guest UUID from `localStorage`, passed as the `user_id` query param — note web user IDs are **strings**, unlike the bot's int IDs.
